...
class Deliverynote extends Controller
{
    ...
    public function index()
    {
		...
		if(input("seeAll")==1){
			header("Location:".url('deliverynote/index'));
		}
		//搜索条件
		$sqlwhere=array("","");
		$allno=input("allno");
		...
		$list = db('bts_delivery_note')->where($sqlwhere[0])->order("makedate desc")->paginate($pagesize, false, ['query' => request()->param(),]);
		$page = $list->render();
		//  页数量
	...
	{
		$field_var='dballno,qno,rno,pino,pono,piid,poid,type,makedate,makename,customname,customid,customaddress,contact,phone,fax,email,deliverydate,itemid,productorderid,num,unit,productctn';
		$field_text='content,mark,markpicture,side,sidepicture,productref,productdes';
		$field_item_var='no_item,itemid,unit,picture,num,productorderid,productctn';
		$field_item_text='productref,productdes';
		...
		{
			$id=input('id');
			$rs = db("bts_delivery_note")->where("id",$id)->select();
			if($data){showjserr("您要编辑的Delivery_Note不存在！");}
			$data=$rs[0];
			$no=newbasicno("temp");
			$data['qno']=$data['no'];
			...
			$udata['no']=$no;
			db("bts_temp")->insert($udata);
			$data = db("bts_delivery_note_item")->where("allno",$data['allno'])->select();
			//mydump($data);
		...
		if($action=="submit")
		{
			...
			//去除非必要的字段
			$allno = $data['dballno'];
			$idata=setsubmitdata("bts_delivery_note",$data);
			//mydump($idata);
			...
			if($data['type']=="new"||$data['type']=='r')
			{
				...
				{
					$idata['no']=$data['qno'];
					$idata['rno']=newtabler("bts_delivery_note","no like '".$data['qno']."'");
				}
				else
				{
					$idata['no']=newbasicno("delivery_note",1);
					$idata['rno']=0;
				}
				$idata['allno']=showtableno("delivery note",$idata['no'],0,$idata['rno']);
				$allno = $idata['allno'];
				$idata['makedate']=now();
				//mydump($idata);
				db("bts_delivery_note")->insert($idata);
			}
			else if($data['type']=="edit")
			{
				...
				//mydump($data);
				db("bts_delivery_note")->where('allno',$data['dballno'])->update($idata);
				db("bts_delivery_note_item")->where('allno',$data['dballno'])->delete();
			}
			...
			for($i=0;$i<count($itemrs);$i++)
			{
				...
				//dump($listitem);
				$lidata=setsubmitdata("bts_delivery_note_item",$listitem);
				//dump($lidata);
				//exit();
				//mydump($data);
				$lidata['allno']=$allno;
				...
				$lidata['time']=now();
				//mydump($lidata);
				db("bts_delivery_note_item")->insert($lidata);
				//dump($lidata);
				//exit();
			}
	...
	//查看delivery note
	public function dyn_show()
	{
		$id =input('id');
		$rs = db('bts_delivery_note')->where('id',$id)->find();
		$itemarr = db("bts_delivery_note_item")->where('no = "'.$rs['no'].'" and rno = "'.$rs['rno'].'"')->select();
		$this->assign('rs',$rs);
