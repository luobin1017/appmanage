...
class Creditnote extends Controller
{
    ...
    public function index()
    {
		...
		if(input("seeAll")==1){
			header("Location:".url('creditnote/index'));
		}
		//搜索条件
		$sqlwhere=array("","");
		$allno=input("allno");
		...
		$list = db('bts_credit_note')->where($sqlwhere[0])->order("makedate desc")->paginate($pagesize, false, ['query' => request()->param(),]);
		$page = $list->render();
		//  页数量
	...
	{
		$field_var='dballno,qno,rno,pino,pono,piid,poid,type,makedate,makename,customname,customid,customaddress,contact,phone,fax,email,currencys,itemid,productorderid,num,unit,price,priceid,amount';
		...
		{
			$id=input('id');
			$rs = db("bts_credit_note")->where("id",$id)->select();
			if($data){showjserr("您要编辑的Credit_Note不存在！");}
			$data=$rs[0];
			$no=newbasicno("temp");
			$data['qno']=$data['no'];
			...
			$udata['no']=$no;
			db("bts_temp")->insert($udata);
			$data = db("bts_credit_note_item")->where("allno",$data['allno'])->select();
			//mydump($data);
		...
		if($action=="submit")
		{
			...
			//去除非必要的字段
			$allno = $data['dballno'];
			$idata=setsubmitdata("bts_credit_note",$data);
			//dump($idata);
			...
			if($data['type']=="new"||$data['type']=='r')
			{
				...
				{
					$idata['no']=$data['qno'];
					$idata['rno']=newtabler("bts_credit_note","no like '".$data['qno']."'");
				}
				else
				{
					$idata['no']=newbasicno("credit_note",1);
					$idata['rno']=0;
				}
				$idata['allno']=showtableno("credit note",$idata['no'],0,$idata['rno']);
				$allno = $idata['allno'];
				$idata['makedate']=now();
				//mydump($idata);
				db("bts_credit_note")->insert($idata);
			}
			else if($data['type']=="edit")
			{
				...
				//mydump($data);
				db("bts_credit_note")->where('allno',$data['dballno'])->update($idata);
				db("bts_credit_note_item")->where('allno',$data['dballno'])->delete();
			}
			...
			for($i=0;$i<count($itemrs);$i++)
			{
				...
				//dump($listitem);
				$lidata=setsubmitdata("bts_credit_note_item",$listitem);
				//dump($lidata);
				//exit();
				//mydump($data);
				$lidata['allno']=$allno;
				...
				$lidata['time']=now();
				//mydump($lidata);
				db("bts_credit_note_item")->insert($lidata);
				//dump($lidata);
				//exit();
			}
	...
	//查看credit note
	public function cn_show()
	{
		$id =input('id');
		$rs = db('bts_credit_note')->where('id',$id)->find();
		$itemarr = db("bts_credit_note_item")->where('no = "'.$rs['no'].'" and rno = "'.$rs['rno'].'"')->select();
